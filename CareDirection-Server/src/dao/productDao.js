...
exports.insertProduct = (Transaction, data, next) => {
  return Transaction(async (connection) => {
    const Query1 = `
        ...
        product_standard2_description, product_standard3_description, product_detail_name, product_detail_value, product_features_name)
        VALUES(
        ...
        "${data.product_detail_value}",
        "${data.product_features_name}"
        )
        `
...
// 제품 상세 그래프 dao
exports.getProductDetailParentGraph = (Transaction, req, next) => {
  return Transaction(async (connection) => {
    ...
    `
    const plusXValue = await connection.query(Query11)
    const result = {
      standardArray,
      change,
      plusXValue,
